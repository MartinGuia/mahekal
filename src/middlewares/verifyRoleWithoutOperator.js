...
const verifyRoleWithoutoperator =  (req, res, next) => { 
    // Body destructuring
    ...
    jwt.verify(token, config.SECRET_KEY, async (err, user) => {
        ...
        if (role.name !== "Administrador" && role.name !== "Gerente Administrador" && role.name !== "Gerente Área") {
...
export default verifyRoleWithoutoperator